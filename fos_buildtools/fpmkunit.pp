...
Type
  ...
  { TNamedItemEnvironment }

  TNamedItemEnvironment = class(TNamedItem)
  private
    //ExtraEnvironmentVariables which could be fetch while compiling
    FExtraEnvironment: TStrings;
    function GetExtraEnvironment: Tstrings;
    procedure SetExtraEnvironment(AValue: Tstrings);
  public
    Property ExtraEnvironment: TStrings Read GetExtraEnvironment Write SetExtraEnvironment;
    Function HaveExtraEnvironment : Boolean;
  end;
...
{$endif cpu_only_overloads}
  ...
  TTarget = Class(TNamedItemEnvironment)
  private
    FInstall : Boolean;
...
{$endif cpu_only_overloads}
  ...
  TPackage = Class(TNamedItemEnvironment)
  private
...
function TNamedItemEnvironment.GetExtraEnvironment: Tstrings;
begin
  If (FExtraEnvironment=Nil) then
    FExtraEnvironment:=TStringList.Create;
  Result:=FExtraEnvironment;
end;

procedure TNamedItemEnvironment.SetExtraEnvironment(AValue: Tstrings);
begin
  If (AValue=Nil) or (AValue.Count=0) then
    FreeAndNil(FExtraEnvironment)
  else
    ExtraEnvironment.Assign(AValue);
end;

function TNamedItemEnvironment.HaveExtraEnvironment: Boolean;
begin
  Result:=(FExtraEnvironment<>Nil);
end;
      ...
      if Defaults.UseEnvironment then
        ...
        begin
          ...
          try
            for i := 0 to GetEnvironmentVariableCount-1 do
              env.Add(GetEnvironmentString(i));
            if APackage.HaveExtraEnvironment then
              Env.AddStrings(APackage.ExtraEnvironment);
            if ATarget.HaveExtraEnvironment then
              Env.AddStrings(ATarget.ExtraEnvironment);
            S:=GetCompilerCommand(APackage,ATarget,Env);
            ExecuteCommand(GetCompiler,S,env);
          finally
            Env.Free;
...
begin
  ...
  Try
    ...
    if APackage.BuildMode=bmBuildUnit then begin
      CompileBuildUnit;
    end;
...
begin
  ...
  try
    ...
    if APackage.InstallProgramPrefix<>'' then
      D:=D+APackage.InstallProgramPrefix+PathDelim;
