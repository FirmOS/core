...
begin
  if not app.CheckAppRightModule(conn,'domain') then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
begin
  if not (app.CheckAppRightModule(conn,'domain') and conn.CheckRight(Get_Rightname('view_domains'))) then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
  ...
  for i := 0 to input.Field('selected').ValueCount - 1 do begin
    ...
    if addrole then begin
      if conn.AddGroupRoles(group.ObjectName+'@'+group.Domain,GFRE_DBI.ConstructStringArray([role.ObjectName+'@'+role.Domain]))<>edb_OK then
        raise EFRE_DB_Exception.Create(StringReplace(StringReplace(app.FetchAppText(ses,'$error_add_role_msg').Getshort,'%group%',group.ObjectName+'@'+group.Domain,[rfReplaceAll]),'%role%',role.ObjectName+'@'+role.domain,[rfReplaceAll]));
    end else begin
      if conn.RemoveGroupRoles(group.ObjectName+'@'+group.Domain,GFRE_DBI.ConstructStringArray([role.ObjectName+'@'+role.Domain]),true)<>edb_OK then
        raise EFRE_DB_Exception.Create(StringReplace(StringReplace(app.FetchAppText(ses,'$error_remove_role_msg').Getshort,'%group%',group.ObjectName+'@'+group.Domain,[rfReplaceAll]),'%role%',role.ObjectName+'@'+role.domain,[rfReplaceAll]));
    end;
  end;
  Result:=GFRE_DB_NIL_DESC;
end;
...
begin
  if not app.CheckAppRightModule(conn,'role') then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
begin
  if not (app.CheckAppRightModule(conn,'role') and conn.CheckRight(Get_Rightname('edit_usergroups'))) then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
var
  ...
  txt         : IFRE_DB_TEXT;
begin
  if not app.CheckAppRightModule(conn,'group') then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
var
  ...
  user        : TFRE_DB_LAYOUT_DESC;
begin
  if not (app.CheckAppRightModule(conn,'group') and conn.CheckRight(Get_Rightname('edit_usergroups'))) then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
begin
  if not (app.CheckAppRightModule(conn,'group')  and (conn.CheckRight(Get_Rightname('edit_grouproles')) or conn.CheckRight(Get_Rightname('edit_usergroups')))) then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
begin
  ...
  for i := 0 to input.Field('selected').ValueCount - 1 do begin
    ...
    if conn.RemoveUserGroups(user.login+'@'+user.Domain,GFRE_DBI.ConstructStringArray([group.ObjectName+'@'+group.Domain]))<>edb_OK then
      raise EFRE_DB_Exception.Create(StringReplace(StringReplace(app.FetchAppText(ses,'$error_remove_group_msg').Getshort,'%group%',group.ObjectName+'@'+group.Domain,[rfReplaceAll]),'%user%',user.login+'@'+user.domain,[rfReplaceAll]));
  end;
  Result:=GFRE_DB_NIL_DESC;
...
begin
  ...
  for i := 0 to input.Field('selected').ValueCount - 1 do begin
    ...
    if conn.ModifyUserGroups(user.login+'@'+user.Domain,GFRE_DBI.ConstructStringArray([group.ObjectName+'@'+group.Domain]),true)<>edb_OK then
      raise EFRE_DB_Exception.Create(StringReplace(StringReplace(app.FetchAppText(ses,'$error_add_group_msg').Getshort,'%group%',group.ObjectName+'@'+group.Domain,[rfReplaceAll]),'%user%',user.login+'@'+user.domain,[rfReplaceAll]));
  end;
  Result:=GFRE_DB_NIL_DESC;
end;
...
begin
  ...
  if session.IsInteractiveSession then begin
    ...
    with user_grid do begin
      ...
      SetDeriveTransformation   (tr_Grid);
      AddRightFilterForEntryAndUser('RF','VIEWDOM','DOMAINID');
      SetDisplayType            (cdt_Listview,[cdgf_ShowSearchbox,cdgf_ColumnDragable,cdgf_ColumnHideable,cdgf_ColumnResizeable],'',nil,'',CWSF(@WEB_UGMenu),nil,CWSF(@WEB_ContentInfo));
    end;
...
var
  ...
  txt         : IFRE_DB_TEXT;
begin
  if not app.CheckAppRightModule(conn,'user') then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
var
  ...
  group        : TFRE_DB_LAYOUT_DESC;
begin
  if not (app.CheckAppRightModule(conn,'user') and conn.CheckRight(Get_Rightname('edit_usergroups'))) then raise EFRE_DB_Exception.Create(app.FetchAppText(ses,'$error_no_access').Getshort);
...
begin
  ...
  if input.FieldExists('SELECTED') and (input.Field('SELECTED').ValueCount>0)  then begin
    sel_guid := input.Field('SELECTED').AsGUID;
    CheckDbResult(conn.FetchUserById(GFRE_BT.HexString_2_GUID(input.Field('selected').AsString),user),'UserContent');
    GFRE_DBI.GetSystemSchemeByName('TFRE_DB_USER',scheme);
...
begin
  ...
  CheckDbResult(conn.ModifyGroupRoles(Get_Groupname_App_Group_Subgroup(ObjectName,'USER'+'@'+domain),GFRE_DBI.ConstructStringArray([Get_Rightname_App_Role_SubRole(ObjectName,'view_users'+'@'+domain)])),'InstallSystemGroupsAndRoles');
  if domain=cSYS_DOMAIN then begin
    ...
    //CheckDbResult(conn.ModifyGroupRoles(Get_Groupname_App_Group_Subgroup(ObjectName,'ROOT'+'@'+domain),GFRE_DBI.ConstructStringArray([Get_Rightname_App_Role_SubRole(ObjectName,'all_rights'+'@'+domain)])),'InstallSystemGroupsAndRoles');

    CheckDbResult(conn.ModifyGroupRoles(Get_Groupname_App_Group_Subgroup(ObjectName,'ADMIN'+'@'+domain),GFRE_DBI.ConstructStringArray([Get_Rightname_App_Role_SubRole(ObjectName,'view_users'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_users'+'@'+domain),
                                        Get_Rightname_App_Role_SubRole(ObjectName,'edit_usergroups'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_groups'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_grouproles'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_domains'+'@'+domain)])),'InstallSystemGroupsAndRoles');
  end else begin
    CheckDbResult(conn.ModifyGroupRoles(Get_Groupname_App_Group_Subgroup(ObjectName,'ADMIN'+'@'+domain),GFRE_DBI.ConstructStringArray([Get_Rightname_App_Role_SubRole(ObjectName,'view_users'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_users'+'@'+domain),
                                        Get_Rightname_App_Role_SubRole(ObjectName,'edit_usergroups'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_groups'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'edit_grouproles'+'@'+domain),Get_Rightname_App_Role_SubRole(ObjectName,'view_domains'+'@'+domain)])),'InstallSystemGroupsAndRoles');
  end;
end;
